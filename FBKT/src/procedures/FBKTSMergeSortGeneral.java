...
public class FBKTSMergeSortGeneral extends RSP {
	...
	public FBKTSMergeSortGeneral(int N, int N0, ArrayList<Double> mu, ArrayList<Double> sigma2,int Allocation,long seed) {
		this.N = N;
		this.N0 = N0;
		this.mu.addAll(mu);
	...
	public void run() {
		...
		for(int i = 0 ; i < k; i++) {
			...
			T.add(N0*1d/k);
			sumFrakX.add(R.nextGaussian()*Math.sqrt(sigma2.get(I.get(i))*(N0*1d/k))+N0*1d/k*mu.get(I.get(i)));
		}
