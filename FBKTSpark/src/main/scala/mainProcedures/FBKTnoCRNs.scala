...
object FBKTnoCRNs {
  def withinCoreSelection(coreID: Int, nCores: Int, nSys: Int, N0: Double, N:Double, param: String, acc_sim_t: Accumulator[Long] ):(Int,Double)={
    val FBKT = new FBKTnoCRNsJ(coreID,nCores,nSys,N0,N,param);
    FBKT.runSystem();
  ...
  def main(args:Array[String]):Unit={
    ...
    while (count < repeat){
      ...
      val coreOutPut = coreID.map(withinCoreSelection(_, nCores, nSys, N0, N, param,accum_sim_t)).cache();
