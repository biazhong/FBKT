//Parameter n0 = initial stage sampling budget / k;
//Parameter n = total sampling budget / k; 
//Parameter param is the problem parameter for the throughput-maximization problem typically we set it to be 20,50,128; 
//Parameter nCores specifies the number of cores to be used to conduct the selection;
//Parameter repeat is the number of macro-replications to estimate the results;
//Parameter omega specifies the varibility in simulation times of the observations.
...
object FBKTCRNsRTime {
  def withinCoreSelection(coreID: Int, nCores: Int, nSys: Int, N0: Double, N:Double, param: String, finalSeed: Double, omega:Double,acc_sim_t: Accumulator[Long] ):(Int,Double)={
    val FBKT = new FBKTCRNsRTimeJ(coreID,nCores,nSys,N0,N,param,finalSeed,omega);
    FBKT.runSystem();
  ...
  def main(args:Array[String]):Unit={
    ...
    while (count < repeat){
      ...
      val coreOutPut = coreID.map(withinCoreSelection(_, nCores, nSys, N0, N, param,seedFinal,omega,accum_sim_t)).cache();
