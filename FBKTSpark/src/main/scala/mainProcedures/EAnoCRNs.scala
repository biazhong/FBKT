...
object EAnoCRNs {
  def withinCoreSelection(coreID: Int, nCores: Int, nSys: Int, N:Double, param: String, acc_sim_t: Accumulator[Long] ):(Int,Double)={
    val EA = new EAnoCRNsJ(coreID,nCores,nSys,N,param);
    EA.runSystem();
  ...
  def main(args:Array[String]):Unit={
    ...
    while (count < repeat){
      ...
      val coreOutPut = coreID.map(withinCoreSelection(_, nCores, nSys, N, param,accum_sim_t)).cache();
