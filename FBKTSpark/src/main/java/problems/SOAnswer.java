...
public class SOAnswer{
	...
	public double getFnVar() {
		...
		return (FnSumSq - size*fn*fn)/(size-1);
	}
	 ...
	 */
	public void addSample(double _fn, double _fn2, int _sz) {
		int newSize=_sz+this.size;
		double newFn = (_sz * _fn + this.size * this.fn)/newSize;
		double newFn2 = _fn2+this.FnSumSq;//(_sz * _fn2 + this.size * this.FnSumSq)/newSize;
		this.fn = newFn;
		this.FnSumSq = newFn2;
		this.size = newSize;
	}
